## Slice
- `image-maintenance flow` from `AdminController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/AdminController.cs`
- Flow: `image-maintenance flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AdminController.cs` (`image-maintenance flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `image-maintenance flow`.
...
## Out Of Scope
- other actions in `AdminController.cs` outside the selected `image-maintenance flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `image-maintenance flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-admin-image-maintenance/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `image-maintenance flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for image-maintenance flow, scope limit to mvc-admin-image-maintenance only, proof target fixture + admin parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for image-maintenance flow, scope limit to mvc-admin-image-maintenance only, proof target bounded parity checks for the exact MVC flow`
