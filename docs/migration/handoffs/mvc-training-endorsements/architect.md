## Slice
- `endorsements flow` from `TrainingController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/TrainingController.cs`
- Flow: `endorsements flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/TrainingController.cs` (`endorsements flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `endorsements flow`.
...
## Out Of Scope
- other actions in `TrainingController.cs` outside the selected `endorsements flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `endorsements flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-training-endorsements/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `endorsements flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for endorsements flow, scope limit to mvc-training-endorsements only, proof target fixture + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for endorsements flow, scope limit to mvc-training-endorsements only, proof target bounded parity checks for the exact MVC flow`
