## Slice
- `telemetry-crop flow` from `FlightsController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs`
- Flow: `telemetry-crop flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs` (`telemetry-crop flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `telemetry-crop flow`.
...
## Out Of Scope
- other actions in `FlightsController.cs` outside the selected `telemetry-crop flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `telemetry-crop flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-flights-telemetry-crop/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `telemetry-crop flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for telemetry-crop flow, scope limit to mvc-flights-telemetry-crop only, proof target route tests + fixture for the single MVC flow`
- Target TODO location: `apps/web/src/components/flight-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for telemetry-crop flow, scope limit to mvc-flights-telemetry-crop only, proof target bounded parity checks for the exact MVC flow`
