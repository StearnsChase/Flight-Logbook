## Slice
- `display flow` from `PrefsController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/PrefsController.cs` (`display flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `display flow`.
...
## Out Of Scope
- other actions in `PrefsController.cs` outside the selected `display flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `display flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-prefs-display/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `display flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for display flow, scope limit to mvc-prefs-display only, proof target service tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/components/app-shell.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for display flow, scope limit to mvc-prefs-display only, proof target bounded parity checks for the exact MVC flow`
