## Slice
- `two-factor flow` from `AuthController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/AuthController.cs`
- Flow: `two-factor flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AuthController.cs` (`two-factor flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `two-factor flow`.
...
## Out Of Scope
- other actions in `AuthController.cs` outside the selected `two-factor flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `two-factor flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `ui parity + auth service tests`.
- the row does not silently absorb unrelated actions from `AuthController.cs`.

## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `ui parity + auth service tests`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-auth-two-factor/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `two-factor flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for two-factor flow, scope limit to mvc-auth-two-factor only, proof target ui parity + auth service tests for the single MVC flow`
- Target TODO location: `apps/web/app/page.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for two-factor flow, scope limit to mvc-auth-two-factor only, proof target bounded parity checks for the exact MVC flow`
