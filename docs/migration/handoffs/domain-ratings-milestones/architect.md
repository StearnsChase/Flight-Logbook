## Slice
- `ratings milestone behavior`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/TrainingController.cs`
- Behavior: `ratings milestone behavior`
...
- `apps/api/src/myflightbook_api/services/ratings/faa_8710.py`: current ratings-domain anchor and nearest milestone-reporting service seam
- `apps/api/tests/test_faa_8710.py`: current ratings proof surface and nearest existing training test anchor

## In Scope
- preserve the exact domain behavior named in the ledger for `ratings milestone behavior`.
...
## Acceptance Criteria
- the current stack has a bounded service or model seam for `ratings milestone behavior` only.
...
## Fixtures And Proof
- store new fixtures or shadow-comparison artifacts under `apps/api/tests/fixtures/domain/domain-ratings-milestones/`.
...
- proof targets:
  - the exact `ratings milestone behavior` boundary matches the legacy source named above.
...
## Escalation Risks
- if the exact `ratings milestone behavior` boundary turns out to span more than one legacy contract surface, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact service or model boundary for ratings milestone behavior, scope limit to domain-ratings-milestones only, proof target fixture + service test for the single domain surface`
- Target TODO location: `apps/api/tests/test_faa_8710.py`
- `TODO (Codex): scaffold the smallest adjacent proof or consumer seam for ratings milestone behavior, scope limit to domain-ratings-milestones only, proof target bounded parity coverage for the exact domain row`
