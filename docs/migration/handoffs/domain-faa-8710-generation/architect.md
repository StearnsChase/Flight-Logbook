## Slice
- `FAA 8710 generation behavior`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/TrainingController.cs`
- Behavior: `FAA 8710 generation behavior`
...
- `apps/api/src/myflightbook_api/services/ratings/faa_8710.py`: current FAA 8710 generation service anchor
- `apps/api/tests/test_faa_8710.py`: current FAA 8710 proof surface

## In Scope
- preserve the exact domain behavior named in the ledger for `FAA 8710 generation behavior`.
...
## Acceptance Criteria
- the current stack has a bounded service or model seam for `FAA 8710 generation behavior` only.
...
## Fixtures And Proof
- store new fixtures or shadow-comparison artifacts under `apps/api/tests/fixtures/domain/domain-faa-8710-generation/`.
...
- proof targets:
  - the exact `FAA 8710 generation behavior` boundary matches the legacy source named above.
...
## Escalation Risks
- if the exact `FAA 8710 generation behavior` boundary turns out to span more than one legacy contract surface, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact service or model boundary for FAA 8710 generation behavior, scope limit to domain-faa-8710-generation only, proof target fixture + service test for the single domain surface`
- Target TODO location: `apps/api/tests/test_faa_8710.py`
- `TODO (Codex): scaffold the smallest adjacent proof or consumer seam for FAA 8710 generation behavior, scope limit to domain-faa-8710-generation only, proof target bounded parity coverage for the exact domain row`
