## Slice
- `callback flow` from `oAuthController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/oAuthController.cs` (`callback flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `callback flow`.
...
## Out Of Scope
- other actions in `oAuthController.cs` outside the selected `callback flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `callback flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-oauth-callback/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `callback flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for callback flow, scope limit to mvc-oauth-callback only, proof target route tests + auth parity for the single MVC flow`
- Target TODO location: `apps/web/app/page.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for callback flow, scope limit to mvc-oauth-callback only, proof target bounded parity checks for the exact MVC flow`
