## Slice
- `aircraft-search flow` from `SearchController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/SearchController.cs`
- Flow: `aircraft-search flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/SearchController.cs` (`aircraft-search flow`)
...
- `apps/api/src/myflightbook_api/api/routes/aircraft.py`: current aircraft route boundary and nearest search and query seam
- `apps/web/src/components/aircraft-composer.tsx`: current aircraft UI anchor for aircraft-search scaffolding

## In Scope
- preserve the exact controller-flow boundary named in the ledger for `aircraft-search flow`.
...
## Out Of Scope
- other actions in `SearchController.cs` outside the selected `aircraft-search flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `aircraft-search flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `ui parity + route tests`.
- the row does not silently absorb unrelated actions from `SearchController.cs`.

## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `ui parity + route tests`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-search-aircraft/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `aircraft-search flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for aircraft-search flow, scope limit to mvc-search-aircraft only, proof target ui parity + route tests for the single MVC flow`
- Target TODO location: `apps/web/src/components/aircraft-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for aircraft-search flow, scope limit to mvc-search-aircraft only, proof target bounded parity checks for the exact MVC flow`
