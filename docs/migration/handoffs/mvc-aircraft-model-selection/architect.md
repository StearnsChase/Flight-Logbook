## Slice
- `model-selection flow` from `AircraftController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/AircraftController.cs`
- Flow: `model-selection flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AircraftController.cs` (`model-selection flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `model-selection flow`.
...
## Out Of Scope
- other actions in `AircraftController.cs` outside the selected `model-selection flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `model-selection flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-aircraft-model-selection/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `model-selection flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for model-selection flow, scope limit to mvc-aircraft-model-selection only, proof target service tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/components/aircraft-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for model-selection flow, scope limit to mvc-aircraft-model-selection only, proof target bounded parity checks for the exact MVC flow`
