## Slice
- `signature-capture flow` from `FlightsController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs`
- Flow: `signature-capture flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs` (`signature-capture flow`)
...
- `apps/api/src/myflightbook_api/api/routes/flights.py`: current flight route boundary for list, detail, share, and download behavior
- `apps/web/src/components/flight-composer.tsx`: current flight UI anchor and nearest web insertion point for flight-facing routes

## In Scope
- preserve the exact controller-flow boundary named in the ledger for `signature-capture flow`.
...
## Out Of Scope
- other actions in `FlightsController.cs` outside the selected `signature-capture flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `signature-capture flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `route tests + fixture`.
- the row does not silently absorb unrelated actions from `FlightsController.cs`.

## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `route tests + fixture`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-flights-signature-capture/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `signature-capture flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for signature-capture flow, scope limit to mvc-flights-signature-capture only, proof target route tests + fixture for the single MVC flow`
- Target TODO location: `apps/web/src/components/flight-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for signature-capture flow, scope limit to mvc-flights-signature-capture only, proof target bounded parity checks for the exact MVC flow`
