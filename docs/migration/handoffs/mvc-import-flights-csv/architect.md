## Slice
- `CSV flight import flow` from `ImportController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ImportController.cs`
- Flow: `CSV flight import flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ImportController.cs` (`CSV flight import flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `CSV flight import flow`.
...
## Out Of Scope
- other actions in `ImportController.cs` outside the selected `CSV flight import flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `CSV flight import flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-import-flights-csv/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `CSV flight import flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for CSV flight import flow, scope limit to mvc-import-flights-csv only, proof target fixture + import tests for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for CSV flight import flow, scope limit to mvc-import-flights-csv only, proof target bounded parity checks for the exact MVC flow`
