...
- Behavior: `route calculation behavior`
...
- `apps/api/src/myflightbook_api/services/geography/airports.py`: current route parsing and airport-distance service anchor
- `apps/api/tests/test_airport_query.py`: nearest geography proof surface already present in the current stack

## In Scope
- preserve the exact domain behavior named in the ledger for `route calculation behavior`.
...
## Acceptance Criteria
- the current stack has a bounded service or model seam for `route calculation behavior` only.
...
## Fixtures And Proof
- store new fixtures or shadow-comparison artifacts under `apps/api/tests/fixtures/domain/domain-geography-route-calculation/`.
...
- proof targets:
  - the exact `route calculation behavior` boundary matches the legacy source named above.
...
## Escalation Risks
- if the exact `route calculation behavior` boundary turns out to span more than one legacy contract surface, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact service or model boundary for route calculation behavior, scope limit to domain-geography-route-calculation only, proof target fixture + service test for the single domain surface`
- Target TODO location: `apps/api/tests/test_airport_query.py`
- `TODO (Codex): scaffold the smallest adjacent proof or consumer seam for route calculation behavior, scope limit to domain-geography-route-calculation only, proof target bounded parity coverage for the exact domain row`
