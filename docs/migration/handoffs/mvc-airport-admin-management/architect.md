## Slice
- `admin-management flow` from `AirportController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AirportController.cs` (`admin-management flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `admin-management flow`.
...
## Out Of Scope
- other actions in `AirportController.cs` outside the selected `admin-management flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `admin-management flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-airport-admin-management/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `admin-management flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for admin-management flow, scope limit to mvc-airport-admin-management only, proof target service tests + admin parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for admin-management flow, scope limit to mvc-airport-admin-management only, proof target bounded parity checks for the exact MVC flow`
