## Slice
- `edit flow` from `AircraftController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AircraftController.cs` (`edit flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `edit flow`.
...
## Out Of Scope
- other actions in `AircraftController.cs` outside the selected `edit flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `edit flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-aircraft-edit/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `edit flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for edit flow, scope limit to mvc-aircraft-edit only, proof target service tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/components/aircraft-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for edit flow, scope limit to mvc-aircraft-edit only, proof target bounded parity checks for the exact MVC flow`
