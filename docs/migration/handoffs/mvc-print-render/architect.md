## Slice
- `render flow` from `PrintController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/PrintController.cs` (`render flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `render flow`.
...
## Out Of Scope
- other actions in `PrintController.cs` outside the selected `render flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `render flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-print-render/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `render flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for render flow, scope limit to mvc-print-render only, proof target pdf fixture + route tests for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for render flow, scope limit to mvc-print-render only, proof target bounded parity checks for the exact MVC flow`
