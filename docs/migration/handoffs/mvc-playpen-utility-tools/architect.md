## Slice
- `utility-tools flow` from `PlaypenController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/PlaypenController.cs`
- Flow: `utility-tools flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/PlaypenController.cs` (`utility-tools flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `utility-tools flow`.
...
## Out Of Scope
- other actions in `PlaypenController.cs` outside the selected `utility-tools flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `utility-tools flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-playpen-utility-tools/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `utility-tools flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for utility-tools flow, scope limit to mvc-playpen-utility-tools only, proof target fixture + utility parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for utility-tools flow, scope limit to mvc-playpen-utility-tools only, proof target bounded parity checks for the exact MVC flow`
