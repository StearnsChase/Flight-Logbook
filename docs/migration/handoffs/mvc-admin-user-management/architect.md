## Slice
- `user-management flow` from `AdminUserController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/AdminUserController.cs`
- Flow: `user-management flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AdminUserController.cs` (`user-management flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `user-management flow`.
...
## Out Of Scope
- other actions in `AdminUserController.cs` outside the selected `user-management flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `user-management flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `service tests + admin parity`.
- the row does not silently absorb unrelated actions from `AdminUserController.cs`.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-admin-user-management/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `user-management flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for user-management flow, scope limit to mvc-admin-user-management only, proof target service tests + admin parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for user-management flow, scope limit to mvc-admin-user-management only, proof target bounded parity checks for the exact MVC flow`
