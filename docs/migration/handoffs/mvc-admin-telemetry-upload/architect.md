## Slice
- `telemetry-upload flow` from `AdminTelemetryController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/AdminTelemetryController.cs`
- Flow: `telemetry-upload flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AdminTelemetryController.cs` (`telemetry-upload flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `telemetry-upload flow`.
...
## Out Of Scope
- other actions in `AdminTelemetryController.cs` outside the selected `telemetry-upload flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `telemetry-upload flow` flow and nothing broader.
...
## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `worker check + fixture`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-admin-telemetry-upload/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `telemetry-upload flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for telemetry-upload flow, scope limit to mvc-admin-telemetry-upload only, proof target worker check + fixture for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for telemetry-upload flow, scope limit to mvc-admin-telemetry-upload only, proof target bounded parity checks for the exact MVC flow`
