## Slice
- `password reset flow` from `AuthController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AuthController.cs` (`password reset flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `password reset flow`.
...
## Out Of Scope
- other actions in `AuthController.cs` outside the selected `password reset flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `password reset flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-auth-password-reset/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `password reset flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for password reset flow, scope limit to mvc-auth-password-reset only, proof target ui parity + auth route tests for the single MVC flow`
- Target TODO location: `apps/web/app/page.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for password reset flow, scope limit to mvc-auth-password-reset only, proof target bounded parity checks for the exact MVC flow`
