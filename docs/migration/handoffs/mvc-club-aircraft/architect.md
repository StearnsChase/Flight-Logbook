## Slice
- `club-aircraft flow` from `ClubController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ClubController.cs`
- Flow: `club-aircraft flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ClubController.cs` (`club-aircraft flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `club-aircraft flow`.
...
## Out Of Scope
- other actions in `ClubController.cs` outside the selected `club-aircraft flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `club-aircraft flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-club-aircraft/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `club-aircraft flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for club-aircraft flow, scope limit to mvc-club-aircraft only, proof target service tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for club-aircraft flow, scope limit to mvc-club-aircraft only, proof target bounded parity checks for the exact MVC flow`
