## Slice
- `image annotation behavior`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ImageController.cs`
- Behavior: `image annotation behavior`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ImageController.cs`
- current-stack file: `apps/api/src/myflightbook_api/services/media/images.py`
- current-stack file: `apps/api/tests/test_image_processing.py`

## Current-Stack Touchpoints
- `apps/api/src/myflightbook_api/services/media/images.py`: current image mutation service anchor
- `apps/api/tests/test_image_processing.py`: current media proof surface closest to image mutation behavior

## In Scope
- preserve the exact media behavior named in the ledger for `image annotation behavior`.
...
## Acceptance Criteria
- the current stack has a bounded media seam for `image annotation behavior` only.
- the proof path satisfies the ledger target for this row: `fixture + media test`.
- no other media row is silently absorbed.

## Fixtures And Proof
- store new fixtures under `apps/api/tests/fixtures/media/media-image-annotation/`.
...
- proof targets:
  - the exact `image annotation behavior` boundary matches the selected legacy source.
...
- if another worker materially changes `apps/api/src/myflightbook_api/services/media/images.py` or `apps/api/tests/test_image_processing.py` before scaffolding begins, re-check conflicting workspace state.

## Feature Handoff Payload
- Target TODO location: `apps/api/src/myflightbook_api/services/media/images.py`
- `TODO (Codex): scaffold the exact media seam for image annotation behavior, scope limit to media-image-annotation only, proof target fixture + media test for the single media row`
- Target TODO location: `apps/api/tests/test_image_processing.py`
- `TODO (Codex): add or extend the smallest adjacent proof or metadata wiring for image annotation behavior, scope limit to media-image-annotation only, proof target bounded parity coverage for the exact media row`
