## Slice
- `visited-airport derivation behavior`

## Source Of Truth
- `MyFlightbook.Airports/VisitedAirport.cs`
- Behavior: `visited-airport derivation behavior`
...
- legacy file: `MyFlightbook.Airports/VisitedAirport.cs`
...
- `apps/api/src/myflightbook_api/services/geography/airports.py`: current airport resolution and route-derivation service anchor
- `apps/api/tests/test_airport_query.py`: nearest current geography proof surface

## In Scope
- preserve the exact domain behavior named in the ledger for `visited-airport derivation behavior`.
...
## Acceptance Criteria
- the current stack has a bounded service or model seam for `visited-airport derivation behavior` only.
...
## Fixtures And Proof
- store new fixtures or shadow-comparison artifacts under `apps/api/tests/fixtures/domain/domain-visited-airport-derivation/`.
...
- proof targets:
  - the exact `visited-airport derivation behavior` boundary matches the legacy source named above.
...
## Escalation Risks
- if the exact `visited-airport derivation behavior` boundary turns out to span more than one legacy contract surface, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact service or model boundary for visited-airport derivation behavior, scope limit to domain-visited-airport-derivation only, proof target fixture + service test for the single domain surface`
- Target TODO location: `apps/api/tests/test_airport_query.py`
- `TODO (Codex): scaffold the smallest adjacent proof or consumer seam for visited-airport derivation behavior, scope limit to domain-visited-airport-derivation only, proof target bounded parity coverage for the exact domain row`
