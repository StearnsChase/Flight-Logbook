## Slice
- `autofill flow` from `FlightEditController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/FlightEditController.cs` (`autofill flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `autofill flow`.
...
## Out Of Scope
- other actions in `FlightEditController.cs` outside the selected `autofill flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `autofill flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-flight-edit-autofill/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `autofill flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for autofill flow, scope limit to mvc-flight-edit-autofill only, proof target service tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/components/flight-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for autofill flow, scope limit to mvc-flight-edit-autofill only, proof target bounded parity checks for the exact MVC flow`
