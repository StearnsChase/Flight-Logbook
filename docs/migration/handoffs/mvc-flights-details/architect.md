## Slice
- `details flow` from `FlightsController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs` (`details flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `details flow`.
...
## Out Of Scope
- other actions in `FlightsController.cs` outside the selected `details flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `details flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-flights-details/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `details flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for details flow, scope limit to mvc-flights-details only, proof target route tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/components/flight-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for details flow, scope limit to mvc-flights-details only, proof target bounded parity checks for the exact MVC flow`
