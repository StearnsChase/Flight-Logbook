## Slice
- `sign-in flow` from `AuthController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AuthController.cs` (`sign-in flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `sign-in flow`.
...
## Out Of Scope
- other actions in `AuthController.cs` outside the selected `sign-in flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `sign-in flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-auth-sign-in/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `sign-in flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for sign-in flow, scope limit to mvc-auth-sign-in only, proof target ui parity + auth service tests for the single MVC flow`
- Target TODO location: `apps/web/app/page.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for sign-in flow, scope limit to mvc-auth-sign-in only, proof target bounded parity checks for the exact MVC flow`
