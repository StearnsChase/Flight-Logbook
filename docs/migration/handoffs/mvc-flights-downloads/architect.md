## Slice
- `download flow` from `FlightsController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs` (`download flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `download flow`.
...
## Out Of Scope
- other actions in `FlightsController.cs` outside the selected `download flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `download flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-flights-downloads/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `download flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for download flow, scope limit to mvc-flights-downloads only, proof target route tests + fixture for the single MVC flow`
- Target TODO location: `apps/web/src/components/flight-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for download flow, scope limit to mvc-flights-downloads only, proof target bounded parity checks for the exact MVC flow`
