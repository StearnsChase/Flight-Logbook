## Slice
- `payment-review flow` from `AdminPaymentController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/AdminPaymentController.cs`
- Flow: `payment-review flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/AdminPaymentController.cs` (`payment-review flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `payment-review flow`.
...
## Out Of Scope
- other actions in `AdminPaymentController.cs` outside the selected `payment-review flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `payment-review flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `route tests + admin parity`.
- the row does not silently absorb unrelated actions from `AdminPaymentController.cs`.

## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `route tests + admin parity`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-admin-payment-review/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `payment-review flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for payment-review flow, scope limit to mvc-admin-payment-review only, proof target route tests + admin parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for payment-review flow, scope limit to mvc-admin-payment-review only, proof target bounded parity checks for the exact MVC flow`
