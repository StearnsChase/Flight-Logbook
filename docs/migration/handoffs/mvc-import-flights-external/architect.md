## Slice
- `external-source import flow` from `ImportController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ImportController.cs`
- Flow: `external-source import flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ImportController.cs` (`external-source import flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `external-source import flow`.
...
## Out Of Scope
- other actions in `ImportController.cs` outside the selected `external-source import flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `external-source import flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-import-flights-external/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `external-source import flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for external-source import flow, scope limit to mvc-import-flights-external only, proof target fixture + import tests for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for external-source import flow, scope limit to mvc-import-flights-external only, proof target bounded parity checks for the exact MVC flow`
