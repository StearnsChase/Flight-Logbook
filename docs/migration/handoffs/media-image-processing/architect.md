...
- Behavior: `image processing behavior`
...
- `apps/api/src/myflightbook_api/services/media/images.py`: current image-processing and metadata-extraction service anchor
- `apps/api/tests/test_image_processing.py`: current media-processing proof surface

## In Scope
- preserve the exact media behavior named in the ledger for `image processing behavior`.
...
## Acceptance Criteria
- the current stack has a bounded media seam for `image processing behavior` only.
- the proof path satisfies the ledger target for this row: `fixture + media test`.
- no other media row is silently absorbed.

## Fixtures And Proof
- store new fixtures under `apps/api/tests/fixtures/media/media-image-processing/`.
...
- proof targets:
  - the exact `image processing behavior` boundary matches the selected legacy source.
...
- `TODO (Codex): scaffold the exact media seam for image processing behavior, scope limit to media-image-processing only, proof target fixture + media test for the single media row`
- Target TODO location: `apps/api/tests/test_image_processing.py`
- `TODO (Codex): add or extend the smallest adjacent proof or metadata wiring for image processing behavior, scope limit to media-image-processing only, proof target bounded parity coverage for the exact media row`
