## Slice
- `image-management flow` from `ImageController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ImageController.cs`
- Flow: `image-management flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ImageController.cs` (`image-management flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `image-management flow`.
...
## Out Of Scope
- other actions in `ImageController.cs` outside the selected `image-management flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `image-management flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-image-management/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `image-management flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for image-management flow, scope limit to mvc-image-management only, proof target route tests + fixture for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for image-management flow, scope limit to mvc-image-management only, proof target bounded parity checks for the exact MVC flow`
