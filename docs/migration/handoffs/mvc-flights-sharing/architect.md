## Slice
- `sharing flow` from `FlightsController.cs`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/FlightsController.cs` (`sharing flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `sharing flow`.
...
## Out Of Scope
- other actions in `FlightsController.cs` outside the selected `sharing flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `sharing flow` flow and nothing broader.
...
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-flights-sharing/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `sharing flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for sharing flow, scope limit to mvc-flights-sharing only, proof target route tests + ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/components/flight-composer.tsx`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for sharing flow, scope limit to mvc-flights-sharing only, proof target bounded parity checks for the exact MVC flow`
