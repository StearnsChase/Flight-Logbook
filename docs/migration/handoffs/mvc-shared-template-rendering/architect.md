## Slice
- `shared-template rendering flow` from `MFBTemplateController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/MFBTemplateController.cs`
- Flow: `shared-template rendering flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/MFBTemplateController.cs` (`shared-template rendering flow`)
...
- `apps/web/src/components/app-shell.tsx`: current shared shell and nearest reusable rendering anchor for common chrome
- `apps/web/src/lib/api.ts`: current client boundary for shared data-loading helpers

## In Scope
- preserve the exact controller-flow boundary named in the ledger for `shared-template rendering flow`.
...
## Out Of Scope
- other actions in `MFBTemplateController.cs` outside the selected `shared-template rendering flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `shared-template rendering flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `ui parity`.
- the row does not silently absorb unrelated actions from `MFBTemplateController.cs`.

## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `ui parity`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-shared-template-rendering/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `shared-template rendering flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for shared-template rendering flow, scope limit to mvc-shared-template-rendering only, proof target ui parity for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for shared-template rendering flow, scope limit to mvc-shared-template-rendering only, proof target bounded parity checks for the exact MVC flow`
