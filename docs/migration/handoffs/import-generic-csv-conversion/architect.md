## Slice
- `generic CSV import conversion flow`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ImportController.cs`
- Flow: `generic CSV import conversion flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ImportController.cs` (`generic CSV import conversion flow`)
- current-stack file: `apps/api/src/myflightbook_api/services/importers/flights.py`
- current-stack file: `apps/api/tests/test_import_generic_csv.py`
...
- `apps/api/tests/test_import_generic_csv.py`: current proof surface for the exact importer conversion row.

## In Scope
- preserve importer-specific header detection, field normalization, row-skip and error behavior, and converted flight payloads for `generic CSV import conversion flow` only.
...
## Acceptance Criteria
- the current importer path is bounded to the exact `generic CSV import conversion flow` conversion surface.
...
## Fixtures And Proof
- add or extend fixtures under `apps/api/tests/fixtures/imports/import-generic-csv-conversion/`.
- prove the exact conversion behavior through `apps/api/tests/test_import_generic_csv.py` and the importer path in `apps/api/src/myflightbook_api/services/importers/flights.py`.
- proof targets:
...
- if another worker changes `apps/api/src/myflightbook_api/services/importers/flights.py` or `apps/api/tests/test_import_generic_csv.py` before scaffolding begins, re-check conflicting workspace state.

## Feature Handoff Payload
- Target TODO location: `apps/api/src/myflightbook_api/services/importers/flights.py`
- `TODO (Codex): scaffold the exact generic CSV import conversion flow conversion path, scope limit to import-generic-csv-conversion only, proof target fixture + import test with source-specific fixtures`
- Target TODO location: `apps/api/tests/test_import_generic_csv.py`
- `TODO (Codex): add or extend source-specific import assertions for generic CSV import conversion flow, scope limit to import-generic-csv-conversion only, proof target golden converted-flight parity for the exact importer row`
