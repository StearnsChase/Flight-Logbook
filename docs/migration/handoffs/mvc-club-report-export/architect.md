## Slice
- `report-export flow` from `ClubController.cs`

## Source Of Truth
- `MyFlightbook.Web/Areas/mvc/Controllers/ClubController.cs`
- Flow: `report-export flow`
...
- legacy file: `MyFlightbook.Web/Areas/mvc/Controllers/ClubController.cs` (`report-export flow`)
...
## In Scope
- preserve the exact controller-flow boundary named in the ledger for `report-export flow`.
...
## Out Of Scope
- other actions in `ClubController.cs` outside the selected `report-export flow` row.
- broad UI redesign or multi-controller consolidation.
- changing the parity target for adjacent MVC rows.

## Acceptance Criteria
- the new stack has a bounded scaffold for the exact `report-export flow` flow and nothing broader.
- the proof path satisfies the ledger target for this row: `fixture + file export checks`.
- the row does not silently absorb unrelated actions from `ClubController.cs`.

## Fixtures And Proof
- capture the exact controller-flow inputs and outputs needed to satisfy the ledger proof target: `fixture + file export checks`.
- store any new fixtures or golden outputs under `apps/api/tests/fixtures/mvc/mvc-club-report-export/`.
...
## Escalation Risks
- the current web app is still at a thin-shell stage, so if implementing `report-export flow` requires rowizing additional route families before this flow can be bounded, stop with `blocked on contract`.
...
- `TODO (Codex): scaffold the exact new-stack service or route boundary needed for report-export flow, scope limit to mvc-club-report-export only, proof target fixture + file export checks for the single MVC flow`
- Target TODO location: `apps/web/src/lib/api.ts`
- `TODO (Codex): scaffold the minimal web entry point, route shell, or client wiring needed for report-export flow, scope limit to mvc-club-report-export only, proof target bounded parity checks for the exact MVC flow`
